---
title: "Tutorial: Basic statitics in R"
...
## A quick catchup and practice for previous sessions

### Data: Import, Export, Generation, vectors, matrices and data frames
 
  
For this session we will use Gene counts data from Lokesh given in the previous session. You can download tthe data here [genes](https://immuntech.github.io/Bioinformatics_Workshop_ImmTech/Data/2019-10-15/gene_counts.tsv) and [metadata](https://immuntech.github.io/Bioinformatics_Workshop_ImmTech/Data/2019-10-15/metadata.tsv).
...
Accessing parts of the data in R can be done in different ways. Lets start by using `data[ , ]` to extract parts of the data. Use left side of the comma for rows and right side of the comma for cloumns. 
...
Lets create a vector of samples you are interested in. To extract the samples from your data simply write the name of the vector on the right side of the comma in the square barckets `[ , ]`.
...
And similarly, to extract you favurite genes by names use following examples.
...
So far we have only printer the extracted data in the consol, this is not very useful if we want to use the extracted data for further analyses. Let us store this data in the "environment" by using `<-` or `=` signs and write it to a file with `write.table()` function.
...
So far we have consolidated our knowledge on how to load, access, subset and save data. This is going to be uesfule in later session.



## Functional programing

R is a **Functional Language** and there is a lot of help available. For almost all necessary data analyses tasks, there are functions available. Most of these functions are well written and have good documentation on how to use them. Often R functions are bundeled in packages. An example of a package is `ggplot2` that is developed for plotting purpose. There are plenty more such as `limma` for analyses of microarray data. There are handy functions to find out what these packages and functions do.
...
If you wnat to know what methods are available for your type of data, you can use `methods` founction as following. 
...
There are several convinience functions and packages available for descriptive statitiscts and analyses in R. We will use some of these and try to apply them on our genes data.

To get values such as mean, standard deviation, variance, min, max, median etc. R has implemented functions in the base package. This packgae is automatically loaded in R when you start the program R or RStudion.
 ...
 **Try to use `rowsd, rowMax, rowMin` etc. to get an idea of the functions available for descriptive statitics.**

If there is a particulr function you are interested in, ask one of the TAs or brown R related webpages.

 ***Mean, Median, Varinace, Standard Deviation, Quantiles, counts, log2, log10, Exponentials***
...
We can extract desired output such as p value, means of groups etc. from this object by using `$` opertator.
...
And confidence intervals by using follwoing code.
